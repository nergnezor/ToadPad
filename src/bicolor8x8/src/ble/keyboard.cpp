 ...
 */
void BleKeyboard::set_keyboard_led(uint16_t conn_handle, uint8_t led_bitmap) {
  (void)conn_handle;
...
void BleKeyboard::setup() {
   ...
   * and max connection interval to 11.25  ms and 15 ms respectively for best
   * performance.
   */
  blehid.begin();
...
void BleKeyboard::keypress(char c) {
  blehid.keyPress(c);
  hasKeyPressed = true;
}
